...
5. **Audience fit.** Check the stated target audience against `audience-map.md`, then check the script is actually written for them. For a student audience: are terms defined on first use, are examples drawn from learning rather than office politics, and does the closing exercise work for someone with no organisation? For a professional audience: does it avoid explaining basics they already know, and does it leave them something to do or something to say? For both: does it lead with the concrete problem and stay usable either way? **A script written at the wrong altitude for its audience fails this item**, however good it is otherwise.
6. **Relevance** — is every sentence serving what it is, why we need it, or how we do it? Flag anything that drifts.
