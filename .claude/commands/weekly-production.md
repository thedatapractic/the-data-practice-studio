...
4. **Draft the full script** for the chosen topic, following the same process as `/new-script`: pick a template (state which + why), **research real, verifiable facts and case studies for this topic** (see CLAUDE.md → "Content generation process" and "Citing real facts, companies, and cases" — officially concluded public cases can be named with real numbers), then draft in that template's shape, personal-example placeholders where needed — never invent an anecdote not already in `producer-memory.md`'s approved bank or `source/` material. This run is unattended, so it can't pause to ask Aji for a fresh personal story the way an interactive `/new-script` run would — instead, flag in the notification (step 9) that the draft would benefit from one, so Aji can supply it before the review pass.
5. **Self-review** the draft using the same process as `/review-script` (ten rules, AI-voice test, intensifier-density scan, raw-material check, runtime check). Fix what can be fixed automatically; leave a placeholder list for anything that needs Aji.
