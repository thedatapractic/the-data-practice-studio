...
8. **Write the purpose sentence first** (CLAUDE.md → "One purpose per video") — one sentence saying what this video exists to make the viewer understand. Put it in the script header. Everything after this has to pass it.
9. Draft the full script in the chosen template's shape, pouring in what steps 5–7 produced. Target ~140 words/minute for a **7–8 minute runtime (~980–1,120 words)** — search-intent and listicle at the lower end, flagship and identity at the upper end. Going long is a failure mode, not a sign of substance: if the material overflows, the excess is a second video, not a longer one.
10. Apply all ten scripting rules as you write, not just at review time — especially rule 6 (concrete beats abstract) and rule 9 (momentum into the next video).
11. Where a personal example is needed but step 6 didn't surface anything and it's not already in the anecdote bank, insert a clearly marked placeholder: `[PLACEHOLDER: presenter's own example of ...]` rather than inventing a fake anecdote.
12. Save the draft to `scripts/drafts/<next-available-number>-<slug>.md`, matching the format of the produced reference script (title, status, **purpose sentence**, runtime, thumbnail text, description/tags, playlist, template used, then the timestamped sections).
13. Update the video's row in `backlog.md` status to `Drafted`. If strong material had to be cut to protect the purpose or the runtime, add it to `backlog.md` as a future video candidate rather than losing it.
