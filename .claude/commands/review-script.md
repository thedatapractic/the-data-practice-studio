...
6. **Principle-consistency check** (see CLAUDE.md → "Leaving the viewer with ethics, principles, and practice," point 4) — state the script's core argument in one sentence. Then list every piece of advice the script actually gives: the takeaway, the exercise, any "here's how you'd fix it" passage, and the CTA's share instruction. Check each one against that sentence and flag anything that contradicts it, or that a viewer could act on in a way the script itself would disapprove of. For this channel specifically: does any advice have an individual acting alone on data rather than routing through a process and a named owner? Does anything imply deletion or removal is the win? Both have been caught on drafts already called finished — treat this as a real check, not a formality.
7. **Purpose check** (CLAUDE.md → "One purpose per video") — find the script's stated purpose sentence in the header; if there isn't one, that's the first finding. Then walk the script section by section and, for each, say in a few words how it serves that purpose. Anything you can't justify against it — a tangent, a second example doing the same job as the first, a credential or framework the video never uses — gets flagged for cutting or compressing, however good it is on its own. Name where the cut material should go in `backlog.md` instead of just deleting it.
8. **Runtime check** — estimate word count against the 7–8 minute band (~980–1,120 words at ~140 words/minute). Over ~1,150 words, the script needs cutting, not trimming — go back to step 7 and find what isn't serving the purpose. Rule 8's cut-15–20%-before-filming pass still applies on top of that.
9. **Rewrite** — produce a full rewritten version of the script that fixes every issue found above, preserving anything that already works. Save it alongside the original as `<original-filename>-reviewed.md` in the same folder.
10. **Placeholder list** — list every spot in the rewritten script (with line/section reference) where only the presenter's own lived experience can fill the gap, e.g. `[PLACEHOLDER: ...]` markers left from drafting, or generic examples that should be replaced with something personal. If the raw-material check in step 5 surfaced a gap only Aji can fill (a better real story, a fact worth verifying), ask directly rather than leaving it as a silent placeholder.
