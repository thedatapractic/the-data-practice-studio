...
7. **Runtime check** — estimate word count and compare to the stated target runtime at ~140 words/minute. Flag if more than ~10% over (rule 8: cut 15–20% before filming).
8. **Rewrite** — produce a full rewritten version of the script that fixes every issue found above, preserving anything that already works. Save it alongside the original as `<original-filename>-reviewed.md` in the same folder.
9. **Placeholder list** — list every spot in the rewritten script (with line/section reference) where only the presenter's own lived experience can fill the gap, e.g. `[PLACEHOLDER: ...]` markers left from drafting, or generic examples that should be replaced with something personal. If the raw-material check in step 5 surfaced a gap only Aji can fill (a better real story, a fact worth verifying), ask directly rather than leaving it as a silent placeholder.
